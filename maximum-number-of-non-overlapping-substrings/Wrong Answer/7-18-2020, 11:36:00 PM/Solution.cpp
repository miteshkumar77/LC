    ...
    vector<string> maxNumOfSubstrings(string s) {
        ...
        for (int i = 0; i < intervals.size(); ++i) {
            for (int j = intervals[i][0]; j < intervals[i][1]; ++j) {
                char c = s[j]; 
                intervals[i][0] = min(intervals[i][0], focc[c - 'a']);
                intervals[i][1] = max(intervals[i][1], locc[c - 'a']);
            }
        }
        ...
        for (int i = 1; i < intervals.size(); ++i) {
            ...
            if (currI[1] < intervals[i][0]) {
                ...
                if (ansIntervals.back()[1] > intervals[i][1]) {
                    ansIntervals.back()[1] = intervals[i][1]; 
                }
            }
        }
        // cout << ansIntervals[0][0] << ' ' << ansIntervals[0][1] << endl; 
