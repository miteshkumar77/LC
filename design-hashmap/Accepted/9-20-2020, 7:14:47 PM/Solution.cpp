...
class HashMap {
public: 
    HashMap(double occupancy_ratio = 0.7, int initial_size = 10) : f(occupancy_ratio) {
        K def; 
    ...
    void set(K key, V val) {
        // cout << "setsearch" << endl; 
        rehash(); 
        // cout << occupied << ' ' << size << ' ' << data.size() << ' ' << values.size() << endl; 
        int init_idx = search(key); 
...
private: 
    ...
    void rehash() {
        ...
        occupied = 0; 
        int i = 0; 
        for (auto [occupied, tomb, key] : n_data) {
            if (occupied) {
