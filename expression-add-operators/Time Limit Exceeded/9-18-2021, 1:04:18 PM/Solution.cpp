...
public:
    ...
    void bt(string box, vector<string>& ans, const string& nums, slli target, slli i, int last) {
        if (i == nums.size()) {
            if (eval(box) == target) {
                ans.push_back(box); 
            }
        ...
        for (string op : ops) {
            bt(box + op + to_string(nums[i] - '0'), ans, nums, target, i + 1, nums[i] - '0'); 
        }
        if (last != 0) {
            bt(box + to_string(nums[i] - '0'), ans, nums, target, i + 1, nums[i] - '0'); 
        }
    }
    ...
    vector<string> addOperators(string num, slli target) {
        ...
        vector<string> ans;
        bt(box, ans, num, target, 1, box[0] - '0'); 
        return ans;
    }
};