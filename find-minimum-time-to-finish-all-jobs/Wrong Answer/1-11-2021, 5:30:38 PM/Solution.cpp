...
public:
    int minimumTimeRequired(vector<int>& jobs, int k) {
        ...
        if (k == 1 || n == 1) {
            return max(accumulate(jobs.begin(), jobs.end(), 0) - *max_element(jobs.begin(), jobs.end()), *max_element(jobs.begin(), jobs.end()));
        }
        int min_max =  accumulate(jobs.begin(), jobs.end(), 0) - 1;
