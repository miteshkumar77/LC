...
public:
    ...
    void propagate(vector<Expr*>& expr_stk, vector<Token>& op_stk) {
                if (expr_stk.size() == 3) {
                    Expr* ret_expr;
                    if ((op_stk[0].t == Token_t::Add || op_stk[0].t == Token_t::Sub) &&
                        (op_stk[1].t == Token_t::Mul || op_stk[1].t == Token_t::Div)) {
                        expr_stk = vector<Expr*>{
                            make_Op(expr_stk[0], op_stk[0].val,
                        ...
                        expr_stk = vector<Expr*>{
                            make_Op(expr_stk[0], op_stk[0].val, expr_stk[1]),
                            expr_stk[2]
                        };
                        op_stk = vector<Token>{op_stk[1]};
                        //ret_expr = make_Op(
